<template>
  ...
  <div class="me-view-body" v-title :data-title="title">
    <article-left :article="article" v-if="article.id!=''"></article-left>
        ...
        <div class="me-view-card">
          ...
          <div class="me-view-author">
            ...
            <div class="me-view-info">
              ...
              <el-button v-if="isFollow" type="success" size="small" @click="cancelFollows()">已关注</el-button>
              <el-button v-else type="success" plain size="small" @click="doFollows()">关注</el-button>
            ...
            <el-button
              v-if=" this.query.authorId == this.$store.state.id"
              @click="editArticle()"
          ...
          <div class="me-view-comment" id="view-comment">
...
<script>
  ...
  import articleLeft from "@/components/articleUtil/articleLeft";
  import default_avatar from '@/assets/img/default_avatar.png'
  import { checkFollow } from "@/api/article";
  import { doFollow } from "@/api/article";
  import { cancelFollow } from "@/api/article";
    ...
    methods: {
      ...
      getArticle() {
        ...
        viewArticle(that.$route.params.id).then(data => {
          ...
          that.query.authorId=data.data.authorId                       
      ...
      checkFollows(){
        ...
        if (that.$store.state.account.length != 0) {
          if (that.query.authorId!=that.$store.state.id) {
            that.query.userId=that.$store.state.id
            console.log(that.query)
            checkFollow(that.query).then(data=>{
              that.isFollow = data.data
              console.log(data)
            }).catch(error => {
            if (error !== 'error') {
      ...
      doFollows(){
        ...
        if (that.$store.state.account.length != 0) {
          if (that.query.authorId==that.$store.state.id) {
            that.$message({type: 'warning', message: '不能关注自己哦', showClose: true})
          }else{
            that.query.userId=that.$store.state.id
            doFollow(that.query).then(data=>{
              that.isFollow = true
              that.$message({type: 'success', message: '关注成功！', showClose: true})
            }).catch(error => {
            if (error !== 'error') {
          ...
          that.$message({type: 'error', message: '请先登录！', showClose: true})
        }          
      },
      cancelFollows(){
        let that = this
        that.query.userId=that.$store.state.id
          cancelFollow(that.query).then(data=>{
            that.isFollow = false
          }).catch(error => {
          if (error !== 'error') {
